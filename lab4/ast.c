...
#include "symbol.h"
#define AST_TRAVERSE_NEXT(node) ast_traverse(node, level, pre_func, post_func, in_func_1, in_func_2)
...
void ast_free(node *ast) {
  ast_traverse(ast, 0, NULL, &ast_free_post, NULL, NULL);
}
...
void ast_print(node * ast) {
  ast_traverse(ast, 0, &ast_print_node, &ast_print_node_post, NULL, NULL);
  fprintf(dumpFile, "\n");
}
  ...
  switch(cur->kind) {
    case SCOPE_NODE:
      if(cur->scope.declarations)
        AST_TRAVERSE_NEXT(cur->scope.declarations);
      if(cur->scope.stmts)
        AST_TRAVERSE_NEXT(cur->scope.stmts);
      break;
    case UNARY_EXPRESION_NODE:
      AST_TRAVERSE_NEXT(cur->unary_expr.right);
      break;
    case BINARY_EXPRESSION_NODE:
      AST_TRAVERSE_NEXT(cur->binary_expr.left);
      AST_TRAVERSE_NEXT(cur->binary_expr.right);
      break;
    case INT_NODE:
      /* Do nothing */
      break;
    ...
    case VAR_NODE:
      /* Do nothing */
    	break;
    case EXP_VAR_NODE:
      AST_TRAVERSE_NEXT(cur->exp_var_node.var_node);
      break;
    case FUNCTION_NODE:
      if (cur->func.args)
        AST_TRAVERSE_NEXT(cur->func.args);
      break;
    case CONSTRUCTOR_NODE:
      AST_TRAVERSE_NEXT(cur->ctor.type_node);
      AST_TRAVERSE_NEXT(cur->ctor.args);
      break;
    case ARGUMENTS_NODE:
      if (cur->args.args)
        AST_TRAVERSE_NEXT(cur->args.args);
      if (cur->args.expr)
        AST_TRAVERSE_NEXT(cur->args.expr);
      break;

    case STATEMENTS_NODE:
      if (cur->stmts.stmts)
        AST_TRAVERSE_NEXT(cur->stmts.stmts);
      AST_TRAVERSE_NEXT(cur->stmts.stmt);
      break;
    case IF_STATEMENT_NODE:
      AST_TRAVERSE_NEXT(cur->if_stmt.condition_expr);
      if (in_func_1) in_func_1(cur, level);
      AST_TRAVERSE_NEXT(cur->if_stmt.if_blk_stmt);
      if (cur->if_stmt.else_blk_stmt) {
        if (in_func_2) in_func_2(cur, level);
        AST_TRAVERSE_NEXT(cur->if_stmt.else_blk_stmt);
      }
      break;
    case ASSIGNMENT_NODE:
      AST_TRAVERSE_NEXT(cur->assignment.variable);
      AST_TRAVERSE_NEXT(cur->assignment.expr);
      break;
    case NESTED_SCOPE_NODE:
      AST_TRAVERSE_NEXT(cur->nested_scope);
      break;

    case DECLARATION_NODE:
      AST_TRAVERSE_NEXT(cur->declaration.type_node);
      if (cur->declaration.expr)
        AST_TRAVERSE_NEXT(cur->declaration.expr);
      break;
    case DECLARATIONS_NODE:
      if (cur->declarations.declarations)
        AST_TRAVERSE_NEXT(cur->declarations.declarations);
      AST_TRAVERSE_NEXT(cur->declarations.declaration);
      break;
    default:
      /* Do nothing */
      break;
